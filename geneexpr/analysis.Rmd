...
for (prefixname in prefixlist) {
  ...
  for (col in seq(nConly)) {
    ...
    points(Cpara_list,rep(s[1,modelname],length(Cpara_list)),type='b',lty=5,lwd=1,pch=col,col=col,cex=0.5)
...
for (prefixname in prefixlist) {
  ...
  for(col in seq(nStatic)){
    ...
    points(npairs_list, rep(ref,length(npairs_list)), type='b',lty=5,lwd=1,pch=col,col=col,cex=0.5)
  }
